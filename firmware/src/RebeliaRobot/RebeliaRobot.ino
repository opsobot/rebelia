...
static const int MAX_TORQUE = 300;  // 30% of max
...
void processStringCmd(const String& cmd) {
  ...
  if (value < FingersController::GraspType::_MAX) {
    ...
    } else if (cmd.substring(0, 4) == "TLIM") {// Comando esempio: TLIM6645
      fc.setMaxTorque(1, cmd.substring(4, 7).toInt() );
      fc.setMaxTorque(2, cmd.substring(4, 7).toInt() );
      fc.setMaxTorque(3, cmd.substring(4, 7).toInt() );
      fc.setMaxTorque(4, cmd.substring(4, 7).toInt() );
      fc.setMaxTorque(5, cmd.substring(4, 7).toInt() );
